...
stage, we will pop nodes one by one from the stack, finding colors for them
based on the interference graph, and reinsert them back into the graph.
...
{\em implicitly}---nodes pushed to the stack are considered to no longer be in
...
* Moves of the virtual register are checked and if the other operand of a move
is available, then it is chosen for the register.
...
Actually spilled registers are marked by having a stack slot allocated.
...
Again, resolution of aliases is important, since the nodes that got merged into
another are not allocated registers at all (never pushed onto the stack) and
...
* Checks whether two nodes are neighbours. E.g. in `are_interfering` and while checking for
existing edge in `add_interference`.
...
reason, usually {\em both} representations were historically~\cite[Chaitin1982]
used for interference graphs and we do the same in our implementation.
...
sparse. So far, we stayed with the bit matrix and in case of need for reducing
memory consumption we can try other tricks like compacting virtual registers as
many temporaries are optimized before register allocation, which can decreasing
$n$ and thus size of the bit matrix considerably.
...
* Adjacency lists are needed for register assignment and physical registers have
fixed assignment, we don't need to iterate over their neighbours.
...
between all physical registers:
...
physical and virtual register into the virtual register, we always combine into
...
registers. In fact trying both heuristic when possible can be beneficial, since
...
that if one of the two registers is physical, it is `u`:
