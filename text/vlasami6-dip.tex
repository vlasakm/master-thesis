...
Most real world programs don't run in isolation. Apart from needing the
...
straightforward to compile into either efficiently or at all.
...
constraints and to compares it to the simplified Tiny86. Knowledge of some
other instruction architecture or at least basic understanding of a processor is
expected, but familiarity with x86-64 is not.

This chapter takes the liberty of approaching some of explaining some of the
properties or advancements non-linearly and often tells little lies. From the
...
presented aren't really true for the actual, original 8086 16-bit processors.
The Intel software developer's manual~\cite[IntelDeveloper] should be taken as
the definitive resource, though I can wholeheartedly also recommend Volume 3 of
AMD's Programmer's Manual~\cite[AMDVol3] (which is focused on the general
purpose instructions and their encodings and I found sometimes more focused and
little bit more approachable), and the unofficial X86 Opcode and Instruction
reference\urlnote{http://ref.x86asm.net/coder64.html}.
...
the history in mind when thinking about the x86-64, and we introduce it here
briefly.
...
which added some new capabilities (like virtual memory) and instructions. A big
...
design, the x86-64 (also called AMD64), which also featured all the 32-bit
additions by Intel, still staying fully backwards compatible. The extension to
64-bits was done in a different way then the extension to 32 bits and brings
...
two address code}, as opposed to three address code that would also have a
non-source destination.

Most operations manipulate registers and memory locations. For example, there is
...
But similar instructions that instead use memory locations do slightly different
things:
...
(`mov`), they do different operations and they are encoded in a different way.
We can view them as different {\em instructions}. Internally, what distinguishes
them is an opcode---some encoding of the instruction as a number, which uniquely
...
operation and we can choose arbitrarily. With this nomenclature in mind, we can
...
\endtt
%
can be called a load instruction, but is actually no more than an instruction
with opcode (137 or 10001001 in binary), which uniquely encodes both the
semantics (copy from second operand to first) and the addressing mode, which has
two operands, first `<reg>`, the other `<reg/mem>`.

The distinction of opcode and mnemonic is important, because in a compiler we
mostly care about the semantics and addressing mode of the operation (i.e. the
...
There are actually 8 registers on the 8086. They have names and some are
...
`ax`) are 16-bit, but separate access to either the upper 8-bits or lower 8-bits
is allowed through the `ah` (\"`ax` high") and `al` (\"`ax` low") registers.
Because of this `ah` and `al` are not real registers, they are just different
names for parts of the existing registers.
...
and called in the alphabetical order `ax`, `bx`, `cx`, `dx`. The registers are
all still general purpose. The categories just signify how they are {\em
usually} used.

Though `bp` and `sp` are usually reserved for special purposes---the {\em call}
stack. The `sp` instruction points to the system stack, which is also used as
the call stack, and `bp` points to the {\em base} of the current functions stack
...
It is no coincidence that there are 8 16-bit registers and 8 8-bit registers.
Most things on the 8086 come in 8s. This stems from the actual encoding of the
...
respective registers, or imply {\em byte} (8-bit) or respectively {\em word}
(16-bit) memory locations in memory.

For example an 8-bit variant of a load instruction has opcode 136 (16-bit
variant has 137) and can look like this:

\begtt
mov al, byte [bx]
\endtt

There are two important things to note: only the first destination became 8-bit,
the memory location is still identified by 16-bit pointer. Also, in the textual
assembly, we can also specify the memory location's operand size with either
...
memory and loaded back, but a lot of other instructions allow their operands to
be memory locations. We usually write memory locations in between brackets (`[`,
`]`) to distinguish them from ordinary uses of registers.
...
are optional. This allows all of the following:
...
{\em instruction pointer} (on 8086 called `ip`). Beware that on x86-64 the
...
useful for {\em position independent code}, often used for libraries---as long
as the whole library is in a continuous piece of memory and uses RIP-relative
addressing, it can actually be located anywhere in memory.
...
Assembler will transform this to the offset to `<label>`, relative to the next
instruction.
\enditems
...
grouping) and often also very similar nature of working. In no category are all
the 8 operations useful for translating usual programs, but here are some
...
Binary arithmetic is nice and simple and allows a memory location as either of
the two operands, e.g. the `add` operation has the following modes:
...
There are two immediate sizes available for 16 bit operations---one can save 8
bits for the immediate if the immediate is small. In these cases when the
...
No special operations are provided for signed or unsigned `add` and other
operations. This is because the produced result is the same for both signed and
unsigned operations---they differ only in how the bits are interpreted (i.e.
unsigned or two's complement) and how an invalid result is detected (either with
...
flags (apart from carry and overflow notably also the zero flag, parity flag or
...
operations---they are {\em long} multiplication and division, i.e. one of the
...
two 16-bit operands, it can actually be used for signed and unsigned
...
* {\em Less than} (`l`). Is true if sign flags is not equal to overflow flag.
* {\em Below} (`l`). Is true if carry flag is 1.
* \dots
\enditems

While there are 16 predetermined ways to interpret either one or two flags, they
can have {\em semantic meaning}, even if they are the same. For example, to
...
check---{\em less than} or {\em below}. The first is intended for unsigned
...
The following instructions have condition codes as part of their opcodes (
`cc` stands for the condition code):
...
immediates, but only `jmp` and `call` allow absolute jumps to address in
register or in memory.

\secc Encoding

All of the opcodes, addressing modes, sizes of operands, are deeply connected
...
behaviour to be changed by a prefix, e.g. the `lock` prefix byte makes some
...
is present for most instructions (though for example, instructions without any
operands don't need it). It encodes up to two operands, one is register and the
other is either register or memory location. The name of the byte stems from its
three components:
...
The four modes are able to encode the following two operands:
...
none of the ModRM modes uses the values from SIB, absence of index or base
cannot be encoded in 3 bits if the registers are also 3 bits. This is {\em
special cases} come in. They are unfortunately too confusing to list here, but
they can be found in~\cite[IntelDeveloper]. Simply put, usually one of the
possible values of a field is reserved for special meaning (e.g. one register in
the `M` part of ModRM means that SIB should be present and consulted). There are
...
exception is using the stack pointer, `sp`, as an index register, which is not
encodable. Fortunately, using `sp` as index register would rarely make sense,
and it can still be used as base.
...
operand were free and could instead be used for extension of the opcode. It is
...
The extension was done in a very straightforward way: the processor supported
a new 32-bit mode, but also a 16-bit mode for backwards compatibility. In the
...
Encoded in exactly the same way as on the 16-bit processors, but with larger
immediates.

The processor was already capable of doing both 16-bit operations and
32-bit operations, but they were multiplexed in the same opcodes. To allow
...
which is able to make memory locations based on 16-bit calculations on 32-bit
...
change only as part of the 32-bit operations.
...
numbered from 8 to 15) and the registers are 64-bit (the full 64-bits are
...
Their solution is the REX (\"register extension") prefix. It is a byte, where
the 4 most significant bits are constant (0100, i.e. 4) and the 4 individual
least significant bits are used as follows:
...
As operand size, also the immediate size stayed at 32 bits. Only a few
...
\endtt
%
which allows 64-bit immediates to be loaded into a register without bit
twiddling. 64-bit immediates were disallowed likely because they both too
large, which negatively impacts the code size, but also unnecessary since most
...
lower 32-bits. This means that 32-bit operations don't care about the upper
32-bits of the 64 bit registers, which is actually very important for modern
super scalar processors. Because with the default 32-bit operand size, if the
...
between accumulator and other registers, if in some cases we are willing to use
...
For the x86-64 architecture, there are two prevalent ABIs:
...
From a perspective of a compiler, the system ABI is not important to internal
functions. But a function which is either called by external code, or which
calls external code must respect the system ABI. The ABI provides standard
...
based on the C ABI, so at least calls to the standard functions have to respect
it.
...
Linux is one of the few operating systems that documents the system call
convention. It is very similar to the calling convention specified by the System
V ABI (it is in fact specified in the same document~\cite[SysVABI]). Only an
extra integer is passed to signify the system call number (because the `syscall`
instruction unlike a normal call doesn't specify \"what to call").
...
convention and doesn't even have to realize the system calls with a
`syscall`.

\sec Comparison with Tiny86

Tiny86~\cite[Strejc2021] is a 64-bit architecture inspired by x86-64. It copies
many of the design decisions of x86-64, which is great for both familiarity (as
FIT students encounter x86-64 in other courses) and as a target, because x86-64
is still very popular this day.

\secc Registers

One of the main differences is the available registers. On x86-64 there are 16
...
which are only accessible through \"special". Tiny86 separates all these 4
...
remainder (confusingly called `mod`) and quotient and there is no long
...
Operations that are distinct for unsigned and signed variants are also
...
Special purpose instructions are available for debugging, as well as reading (from
standard input) and writing (to standard output) numbers. These are implemented
by the Tiny86 VM with calls to C++ standard library.
...
\endtt
%
where `<mem>` is the following, where all components are optional:
...
Tiny86, even those specifying memory locations are 64-bit. This makes it much
more flexible for access to array elements then x86-64, which allows the scale
...
operand specifications are allowed, so the complexity is limited to the
...
features, but irregularities, which stress compiler capabilities.
...
With intermediate representations like trees, DAGs, e.g. with value-based SSA on
a control flow graph (see section~\ref[sec:SSA-value]) the operations and their
...
Even instructions can be represented as graph based patterns. For example an
`add` instruction which accepts two registers and produces a result in a
register, can be represented as a node (the result of the addition) with two
children nodes (the operands). Other pattern can represent a similar
instruction, but with a different addressing mode, e.g. an `add` of a register
...
of instructions, but not necessarily their full ordering---any order resulting
from a bottom-up walk is a possible final instruction sequence.

The nodes in the instruction patterns are typed by the types of operands of the
x86-64 architecture. This captures has gradually nodes in the original graph are
covered by instruction patterns and the produced values themselves may be used
as operands of other instructions. For the x86-64 architecture for example, at
least the following fundamental types are required:
...
so this can be captured by a pattern which \"retypes" the node.
%TODO figure
...
is not produced by anything. Additionally a \"retyping" no-op pattern which
accepts a base pointer and produces a register will then allow the frame
pointer to be usable like registers produced by other patterns like loads of
constants, arguments or results of arithmetic operations.
...
forms sometimes suffice and we stick to the three categories described
...
arithmetic operations produce on result from multiple operands.
...
longer has a single parent, but can have multiple. Directed acyclic graphs
...
control flow graphs. Most advanced instruction selection for a whole procedure
...
for any architecture provided a suitable set of patterns and instructions that
correspond to them.
...
The expensive preprocessing is done at compiler-compile time an thus the
generated parsers are fast. Additional predicates (e.g. checks of small
constants) and actions can be added to the LR parsing through the use of {\em
attributed grammars}~\cite[Knuth1968].

A fundamental problem LR parsing in the area of instruction selection is, that
it expects a linear sequence of terminals, thus the tree structure has to be
...
of reduce-reduce and shift-reduce conflicts is needed. Often shifts are
preferred for reductions, because they are likely to result in bigger matches.
...
matching.
* Apply the algorithm recursively for each child induced by the pattern.
...
ordering. {\em Large} patterns are tried first, but attention is also paid to
their cost. Larger patterns are usually better, because as they implement more
...
R parsing, a classic bottom-up approach, is not that fit for actual application
for trees or DAGs, while a naive top-down approach seems very unoptimal, due to
lack of any backtracking. However, better approaches for bottom-up instruction
selection.
...
third pass. The conflicts that hindered the LR parsing approach are gone---the
...
restrictions on the grammars, and a similar tool `iburg`~\cite[Fraser1992b]
...
subexpressions), then it is possible to either:
...
duplication repeats computations.

A notable application of duplication is Ertl's~\cite[Ertl1999], which is able
to reduce the need for duplication for a class of DAGs. While edge splitting has
...
support DAG {\em patterns}, which are required for multi-output instructions
like `idiv`.
...
The dependencies among instructions as selected by the instruction selection
...
itself), but also memory operations. Loads and stores to memory can be very
limiting to the ordering, alias analysis may be able to tell which stores and
loads are safe to reorder, but writes to memory are always possible, which can
make any reordering hard.

Instruction scheduling then produces a {\em linear order} of the instructions,
where the dependencies are honored, but the instructions themselves are now able
to make better use of machine resources.
...
operations take the same time, any ordering will produce the same sum.
...
instruction in the pipeline as there are pipeline stages and progress is made on
each instruction in each stage of the pipeline. This is comparable to to an
assembly line in a car factory. In a usual pipelined processor we can imagine
...
forwarded} to the stage 2 at the same cycle, but in a real pipelined processor
there is a higher number of stages, which makes dependencies of adjacent
instructions much more problematic.
...
* {\em Assign priorities}. The assigned priorities are used to guide the
scheduler.
* {\em Schedule operations}. A {\em ready list} containing the instructions
that can execute in the current cycle is maintained and the scheduling continues
until it becomes empty. Instructions are removed from this list, until it
becomes empty, at which point based on the cycle count and prepared operands,
new instructions can become ready.
\enditems
...
though each supports only certain kinds of operations. Thus many operations can
be executed in parallel.
* Issuing of multiple instruction in a single cycle. Multiple execution units
are kept saturated, because many operations can start their execution at once.
\enditems
...
This makes {\em static} instruction scheduling in a compiler a lot less
...
of execution units and have high latency),
...
problem, it is possible to leverage efficient methods for solving other problems
through a {\em reduction}.

The methods using reduction fully transform the register allocation problem to
an instance of another problem, often use an external generic solver and then
translate the result back into a register allocation. This is a bit different
than using the graph coloring register allocators, which use the ideas of graph
...
Main interest of this thesis is a back end. As introduced in
...
we show in later sections, the compiler has to know exactly which instructions
...
also handles {\em relocations}, i.e. it adds notes to either the static or
...
implement it (see also section~\ref[sec:runtime-external]).
...
For the assembler, we chose NASM\urlnote{https://www.nasm.us/}. It is assembler
intended for assembly written by humans and as such supports very readable
syntax, unlike other assemblers like the GNU assembler.
...
* How easy it is to build and distribute the compiler.
\enditems
...
in. The language is improving much in each revision of the standard.
...
Ultimately, we decided to implement the software in C. C can be run and compiled
basically anywhere. It doesn't support any data structure or paradigm nicely,
but this often means that we are able to choose the data structures most
suitable to our exact problem. Everyone studying a TinyC compiler is also
...
Since we want to use the representation for instructions both {\em
before} and {\em after} register allocation, and also need to express machine
constraints we have to be able to express physical register as well as virtual
...
* Physical registers are named symbolically through an enum as `R_XXX` and are
...
Programs output by our compiler like all programs will need to interact with the
operating system in two crucial ways:

\begitems
* The operating system needs to transfer the control to an entry point of the
program.
...
system (e.g. a call to the `exit` function of standard C). As
mentioned in~\ref[sec:x86-os], on most platforms the C library is the {\em only}
...
through a C library, such as glibc.
...
doesn't call `main` nor prepares its arguments. This startup is also handled by
the {\em runtime component}, which is linked into the application.
...
language needs to get extended, since as of now it doesn't support calls to
...
return value is be saved into `rax`.
...
linked object files or libraries.
...
This can then be assembled into an ELF object file with `nasm`:

\begtt
nasm -f elf64 -o prog.o prog.asm
\endtt

The final executable can be linked together with the system linker `ld`:
...
executable, the ELF interpreter (\"dynamic linker") is set to its standard path.
The following three ELF files are linked together:

\begitems
* `Scrt1.o`, which contains the `_start` entry point. `_start` is marked as
entry point by the linker's default linker script. The final address of `_start`
will be the ELF entry point, where the execution will start.
...
`gcc` command for the GCC compiler. With the driver, it is also much easier to
...
An equivalent program can be created without running the C library. For
...
arguments, the kernel won't read the other registers that we set. The argument
registers don't have to be preserved according to the C calling convention,
so it doesn't matter that we change them. The read from the stack is benign,
even if it reads an invalid value---if it isn't a valid value, because there are
less than 6 arguments, the kernel won't look at it.
...
no more than 7 we understand them (and there shouldn't actually be more than 7).
...
details from the users. With the use of the wrappers, entire C library could be
implemented.
...
some edges needlessly inserted by our simplistic front end. Sometimes in the
process of redirecting control flow we introduce {\em critical edges}. We
actually split them later (see section~\ref[sec:imp-critical-edge]), but keeping
them unsplit at least temporarily allows for simpler control flow on which we do
the next optimization.

\secc Simple alias analysis

We perform a very weak form of \"alias analysis" on stack slots (what used to be
TinyC local variables), which lets us often transform them into values
respecting SSA, and remove the load and store operations that semantically
happen for all C local variables.
...
is {\em aliased} and written to by other stores. This is very weak, as it aborts
...
use Braun's method~\cite[Braun2013], which is simple and
intuitive---$\phi$-function operands are inserted by querying predecessors.
Handling of cycles and speed are achieved with elegant memoization.
...
starting point for both. Our compiler itself has only few middle end optimizations (
