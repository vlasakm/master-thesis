...
this way often the process can finish in less than 3 iterations.
...
definition. This new temporary can be procolored and it is not going to clash
with any other similar precolored use, since it has short range, just like other
the other newly introduced precolored temporaries and importantly their live
ranges {\em don't overlap}. In the previous example, this would look like this:

\begtt
mov t5, t2
sal t1, t2 // precolor t5 to cl
mov t6, t4
...
Here it was possible to keep all values in registers. The first move can be
...
here. But avoiding uncolorable graphs with splits means a lot of move
instructions, which if allocated different registers, will not be optimized by
peephole optimization and thus will incur often unnecessary overhead. We
...
realized the need for coalescing. His solution was to 
