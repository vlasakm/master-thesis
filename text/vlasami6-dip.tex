...
written a compiler backend yet.
...
* Since programs rarely live in vacuum, the produced code should be able to
interface with the operating system and foreign code.
\enditems

After careful evaluation of the thesis task and other circumstances we set the
following goals for the implementation of a TinyC backend for x86-64 and a
runtime:
...
Since this backend will compile to real machine code, comparisons with other
compilers will be possible and the relative performance of our solution should
be evaluated.
...
The popularity of SSA for middleend optimizations and recent developments in the
field of register allocation on SSA form make it very appealing even for a
backend. In theory the same SSA representation can be used even for the backend.
In practice, this can't work well in an optimizing compiler, which should be
able to optimize based on the machine architecture at hand. 



The TinyC frontend used in the NI-GEN course
...
The entire middle end is based on control flow graphs (CFGs) the idea of {\em
...
opposed to for example something like a `std::list<T>` in C++), which is the
most convenient way for representing linked lists in C.
...
function, thus we need to handle it differently. We also often don't want to
iterate over the blocks in some random order they were created in. But we want
to iterate over them in an order that is beneficial for performed analysis and
optimization passes. Often this order is either reverse postorder (where all
non-cyclic successors are visited {\em after} the block itself) or postorder
(where a block is visited {\em after} all its non-cyclic predecessors).
Postorder is easily computed with depth first search, and an array with
(pointers to) basic blocks is stored in functions. Iteration over the postorder
either is possible also easily possible in reverse, so in a way a reverse
postorder is simultaneously also available. TODO cite dragon book.
...
is no initializer (`init` is `NULL`), then the global variable should be
zero initialized. This is the same behavior as in the C programming
language.\fnote{If a global variable were to be initialized with a non-constant
value it would be questionable {\em when} the value should actually be
evaluated---for example the global variables should certainly be initialized
before `main` runs, but in what order, and how to run code before `main`? For
instance C++ allows non-constant initializers and runs them through hooks
running before `main` offered by the C runtime which is the one calling `main`.}
...
To save space, `kind`, `subkind` and `mode` are all small integers. And while
