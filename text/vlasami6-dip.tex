...
Naturally, the back end will be usable by other front ends. We aim to 
...
used differently and don't support the same features. TinyC for example, doesn't
support calls into the operating system at all, and a {\em runtime} component
that allows TinyC to use system resources as well as to just run has to be
provided and integrated into TinyC.
...
area of compiler back ends (chapter~\ref[chap:state-of-the-art]), and a
close look at the design and implementation of an x86-64 backend for TinyC and
...
and looked at an implementation for a back end translating the TinyC programming
language into it.
...
The implemented backend is based on existing substantial research in the area of
compiler back ends. However, due to many specialities, irregularities or
limitations of the x86-64 architecture, there are other challenges, which are
sometimes which are not always catered for by the publications on topics like
