...
In the end, the operand encoding is fairly regular. Encoding is more complex
because of the special cases, but all useful operand specifications are allowed,
so the complexity is limited to the encoder/decoder, and not the compiler, which
sees the operations as fairly orthogonal.
...
We briefly introduce the history, because the 


extension to a family of earlier 32-bit architectures, commonly called x86. The
x86 itself
...
REX prefix byte (see section~\ref[sec:x86-rex]).
