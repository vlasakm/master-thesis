...
simple step, but of multiple, iterated {\em phases}, which are shown in
figure~\ref[fig:todo], and explained in the following subsections.
...
* {\em Liveness analysis}. Before we are able to construct the interference
...
This \"cascading" effect of dead instructions is not
that common---we much rather prefer to always remove instruction in peephole
patterns where we find them dead, because these removals change what falls into
a peephole window and thus may lead to other optimizations---

\secc Practical findings

move forwarding - surprisingly important

`inc` --- not worth it

deletions of long distanced values can bring other optimizations
