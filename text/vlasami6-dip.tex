...
register allocation (not assignment) may for example even try to purposefuly do
...
first notable use of the technique is by Chaitin~\cite[Chaitin1981]. The core of
the idea is to construct an interference graph from the interferences of
pseudoregisters---all pseudoregisters are nodes in the graph and there is an
edge between pseudoregisters, if and only if they interfere. Then on this graph
we aim to find a {\em coloring}---mapping of nodes to colors, such that
no neighbouring nodes get the same color. In our case the colors ultimately
constitute the machine registers, because edges designate interference, it is
guaranteed that no pseudoregisters that interfere are assigned the same
register.

If we have $K$ machine registers available, then we are looking for a $K$
coloring---the coloring needs to use at most $K$ colors (registers). We call the
minimum number of colors a graph can be colored with it's chromatic number.
Obviously, no matter how $K$ is big, there are always graphs with a bigger
chromatic number (which corresponds to the fact there we always could have a
program needing more physical registers at the same time then we had available).

By reducing the register allocation to graph coloring we may have seemingly not
gained much, since graph coloring is an NP-complete
...
because there are less neighbours than colors, even if all neighbours used
different colors, there would still be a free color left.
This simple, yet important observation is the base for their and subsequent
techniques.

Since incorporating a node with degree less than $K$ (an {\em insignificant}
...
Chaitin's algorithm this phase is understandably called {\em simplify} and the
removed low degree nodes are pushed on the stack. In the final stage, {\em
...
all saving---it is still likely that there will be nodes left with high degrees.
TODO what Chaitin does exactly?
...
and new pseudoregisters were introduced to accomodate spill code, the
liveness analysis and building of interference graph have to be repeated as
well. Then simplification can be tried again. The entire process is repeated
until the simplification is able to reduce the graph to an empty graph, which is
trivially colorable. At this point the invariant of pushing only nodes of
insignificant degrees guarantees us that every node popped from top of the stack
will have at least one free color.

But, it is possible to do better. The fact, that a node has a {\em significant}
number of neighbours doesn't mean, that it will be uncolorable in the assignment
stage. There is a chance that all of the already colored neighbours will be
assigned less than $K$ colors (TODO: figure), in that case the popped node could
still be colored even though when it was pushed it was significant. Because of
this, we may optimistically proceed to even remove and push high degree nodes on
to the stack, without pesimistically spilling them in the simplify phase. If in
the assignment phase it turns out that there isn't a color left for the popped
node, we spill it only then. When we push a high degree node in the simplify
phase, it can lead to other nodes being insignificant and thus trivially
colorable. This strategy is called {\em optimistic coloring} and was devised by
Briggs~\cite[BriggsXXXX].
